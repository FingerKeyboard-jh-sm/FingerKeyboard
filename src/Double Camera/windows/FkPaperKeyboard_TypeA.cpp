...
void FkPaperKeyboard_TypeA::setKeyMaskBottomImage(IplImage* srcImage, IplImage* dstImage){
	IplImage* tempImage = cvCreateImage(cvGetSize(srcImage), IPL_DEPTH_8U,1);
	cvCopy(keyboardMaskBottom, tempImage);
	cvCopy(srcImage, dstImage);
	CvMemStorage *storage = cvCreateMemStorage(0);
	CvSeq * contour;
	cvFindContours(tempImage, storage, &contour, 88, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);
	cvDrawContours(dstImage, contour, CV_RGB(30,30,200), CV_RGB(0,0,0), 0, -1, CV_AA);
	cvReleaseImage(&tempImage);
}
